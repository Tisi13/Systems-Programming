...
int main(int argc, char *argv[]) {
    ...
    // parse arguments
    if (argc < 3) {
        printf("error : called with incorrect number of parameters\nusage : %s <server_name/IP> <filename> <filter_name> <[filter_parameter]> \n",
               argv[0]);
        return -1;
    }
...
//            {"breadth",   required_argument, 0,  'b' },
        ...
        int long_index =0;
        while ((opt = getopt_long(argc, argv,"cs",
                                  long_options, &long_index )) != -1) {
            switch (opt) {
                case 'c' : change_rate = 0;
                    break;
                case 's' : param = atof(optarg);
                      change_rate=0;
                      break;
...
//                    exit(EXIT_FAILURE);
        ...
        if (change_rate == 0) {
            printf("hello %f\n", param);
            spacecats.sample_rate = param*spacecats.sample_rate;
